...
const CastCharacters: FC<CastCharactersProps> = ({ cast, count,dropdow }) => {
  ...
  return (
    <>
      <div className="flex items-center justify-center relative my-8 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 p-6 rounded-lg shadow-lg">
        {Array(count + 1)
          .fill(1)
          .map((item, index) => {
            return (
              <div key={index}>
                {index < count && (
                  <div
                    key={index}
                    className="w-20 h-20 rounded-full overflow-hidden border-4 border-white absolute shadow-xl transform hover:scale-110 transition-transform duration-300 ease-in-out"
                    style={{
                      left: `${index * 50}px`,
                      zIndex: count - index,
                    }}
